...
test("Episodes  data when rendered with new Episode", () => {
// Arrange:

const {queryAllByTestId, getAllByTestId, rerender} =  render(<Episodes episodes={[]}/>); 
const initialEpisodes = queryAllByTestId(/episo/i);
expect(initialEpisodes).toHaveLength(0);
expect(initialEpisodes).toEqual([]);
 // Act:
rerender(<Episodes episodes="" episodes={data} />);
    const episodes = getAllByTestId(/episo/i);
    // Assert:
